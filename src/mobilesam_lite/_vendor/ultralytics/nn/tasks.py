...
def torch_safe_load(weight):
    ...
    try:
        ...
        if 'Weights only load failed' not in str(e):
            raise
        LOGGER.warning(
            f"WARNING ⚠️ {weight} uses a checkpoint format that requires full unpickling with PyTorch 2.6+."
            "\nRetrying with weights_only=False because this project expects trusted local checkpoints."
        )
        return torch.load(file, map_location='cpu', weights_only=False), file  # load legacy full checkpoint
        ...
        try:
            return torch.load(file, map_location='cpu'), file  # load
        except pickle.UnpicklingError as load_error:
            if 'Weights only load failed' not in str(load_error):
                raise
            LOGGER.warning(
                f"WARNING ⚠️ {weight} still requires full unpickling after dependency installation."
                "\nRetrying with weights_only=False because this project expects trusted local checkpoints."
            )
            return torch.load(file, map_location='cpu', weights_only=False), file  # load legacy full checkpoint
